...
/* Functions for handling timer sleep */
void
thread_wakeup (void)
{
  struct list_elem *e;
...
bool
thread_compare_priority (const struct list_elem *e1, const struct list_elem *e2, void *aux)
{
  struct thread *t1, *t2;
...
thread_check_exchange (void)
{
  ...
  for (e = list_begin (&ready_list); e != list_end (&ready_list);
    ...
    {
      struct thread *t = list_entry (e, struct thread, elem);
      if (thread_calc_priority(t) > thread_get_priority ())
        {
          thread_yield ();
          break;
	}
               ...
               thread_func *function, void *aux) 
{
  ...
  /* Compare priority to current thread */  
